...
This page reflects **`react-native-uismith`** as shipped from source (`src/index.ts`). Runtime theme provider/hooks are available, plus **`Button`** and **`Typography`** primitives.
...
## Runtime components (Phase 4)

### `Typography`

- `forwardRef` to React Native `Text`
- Resolves `variant` from `tokens.core.typography.presets`
- Resolves semantic `color` role from active theme slice

### `Button`

- `forwardRef` to React Native `Pressable`
- Supports `variant`, `size`, `intent`, `disabled`, `loading`, `fullWidth`, icons, and `iconOnly`
- Uses theme `sizeMetrics` / `labelPresetBySize` and current semantic map
- Applies press scale from `motionPresets.Button.press.scale`
...
Runtime `Button` export uses this exact prop type.
...
Runtime `Typography` export uses this exact prop type.
